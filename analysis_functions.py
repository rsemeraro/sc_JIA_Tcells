...
def distro_barplot(data_in, xx, yy, xlabel= '', ylabel='', save=False, reorder=None):
    ...
    try:
        ...
        if list(xcolors).count(list(xcolors)[0]) == len(list(xcolors)):
            xcolors = sc.pl.palettes.vega_20
    except:
        xcolors = sc.pl.palettes.vega_20
    if reorder is not None:
        ...
        l = xcolors[:N]
    my_cmap = colors.ListedColormap(l)
    ...
    cbar = plt.colorbar(sm, orientation='horizontal', pad=0.01, aspect=4*N)
    cbar.set_label(xlabel, size = 13)
    if reorder is not None:
        labels = reorder
    else:    
    ...
    ax.set_xlim(0, len(data_in.obs[xx].cat.categories))
    ax.set_ylim(0, 1)
    plt.legend(SexRatio_inCluster, ncol=1, fancybox=False, shadow=False, fontsize='big', bbox_to_anchor=(1.003, 1.02), prop=font_prop, frameon=False)
    if ('/' in xx):
        xx=xx.replace('/','-')  
    elif ('/' in yy):
        yy=yy.replace('/','-')
...
def cell_cycle_info(adata, model):
    ...
    sc.pl.pca_scatter(fulldata_cc_genes, color='phase', palette=phase_cols)
    sc.pp.regress_out(fulldata_cc_genes, ['S_score', 'G2M_score'])
    sc.tl.pca(fulldata_cc_genes)
    sc.pl.umap(fulldata_cc_genes, color=['S_score', 'G2M_score'], use_raw=False)
    if 'batch' not in adata.obs:
        sc.pl.umap(fulldata_cc_genes, color=['phase'], use_raw=False)
        sc.pl.pca_scatter(fulldata_cc_genes, color='phase')
    else:    
        sc.pl.umap(fulldata_cc_genes, color=['phase', 'batch'], use_raw=False)
        sc.pl.pca_scatter(fulldata_cc_genes, color='phase')
...
def sc_normalization(adata, counts=1e6, res=0.5): 
    ...
    with localconverter(robjects.default_converter + pandas2ri.converter):
        ...
        sb.distplot(size_out, bins=50, kde=False)
        plt.show()
